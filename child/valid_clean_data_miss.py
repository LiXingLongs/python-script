    ...
    while(curr < total):
        ...
        for data in collection.find().skip(curr).limit(LIMIT_NUM):
            ...
            try:
                ...
                # 清洗需要转换的数据
                patternStr = "resource.hivoice.cn/dcs-resources"
                isValid = patternStr in dataStr
                if isValid:
                    # 将有金山云连接内容商存起来
                    dataOriginSet.add(data.get("dataOrigin"))
                    idSet.add(data.get("_id"))
