    ...
    while(curr < total):
        ...
        for data in collection.find().skip(curr).limit(LIMIT_NUM):
            ...
            try:
                ...
                # 匹配字符串
                if "dcs-resources.ks3-cn-shanghai.ksyun.com" in dataStr:
                    # 清洗需要转换的数据
                    dataStr = dataStr.replace("dcs-resources.ks3-cn-shanghai.ksyun.com", "resources.hivoice.cn")
                    # 转换回对象
            ...
            # 批量执行更新操作
            if (match_num % LIMIT_NUM == 0 and exe_flag) or (curr >= total and exe_flag):
                bulk.execute()
                logger.info("当前执行条数:" + str(match_num))
                bulk = collection.initialize_ordered_bulk_op()
                exe_flag = False
        logger.info("当前条数:" + str(curr))
